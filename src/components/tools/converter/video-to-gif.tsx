...
import { Upload, Download, FileVideo, RefreshCw, AlertCircle, Loader2 } from 'lucide-react';
...
export function VideoToGif() {
    ...
    const convertToGif = async () => {
        ...
        try {
            ...
            console.error(err);
            setError('Error converting video to GIF.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="max-w-3xl mx-auto space-y-8">
            <div className="text-center space-y-4">
                ...
                <p className="text-lg text-slate-500 max-w-xl mx-auto">
                    Convert MP4, WebM, MOV, and AVI videos to high-quality animated GIFs directly in your browser.
                </p>
            </div>
            ...
            <Card className="p-8 border-slate-200">
                {!loaded && !error ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-8 bg-red-50 rounded-xl text-red-600 border border-red-100">
                        ...
                        <p className="text-sm mt-2 text-red-500">Note: This tool requires a modern browser with WebAssembly support.</p>
                    </div>
                ) : (
                    <div className="space-y-8">
                        ...
                        >
                            ...
                            >
                                <div className="h-16 w-16 bg-white rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center mb-4">
                                    {file ? <FileVideo className="h-8 w-8 text-indigo-600" /> : <Upload className="h-8 w-8 text-slate-400" />}
                                </div>
                                {file ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        <p className="text-sm text-slate-500 mt-1">MP4, WebM, MOV, AVI (Max 50MB recommended)</p>
                                    </>
                                )}
                            </label>
                        </div>
                        ...
                        {gifUrl && (
                            <div className="mt-8 bg-slate-50 rounded-2xl p-4 border border-slate-200">
                                ...
                                <img src={gifUrl} alt="Converted GIF" className="max-w-full h-auto mx-auto rounded-lg shadow-sm" />
                            </div>
                        )}
